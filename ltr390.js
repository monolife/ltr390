...
LTR390.prototype.readLight = function (cb) {
    var self = this;
    if (!utils.exists(cb)) {
    ...
    self.wire.readBytes(self.options.command, self.options.length, function (err, res) {
        if (utils.exists(err)) {
            if (self.verbose)
                console.error("error: I/O failure on LTR390 - command: ", self.options.command);
            return cb(err, null);
        }
